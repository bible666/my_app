select distinct agent_code --,policy_type,pay_to,premium_amount,premium_ordinary,tot_rider_premium_amount,payment_mode,first_name,last_name,phone1,policy_no,product_name,customer_province_name
from (
	select ipm.* , 
		case when 
	...
	from (
		...
		where-- agent_code = cast(:agent_code as text) 
			--and 
			case when lower(policy_type ) = 'o' then lower(rc_status) in ('b','c') else true end 
			and pl.apl_flag = 'Y'
	) ipm 
